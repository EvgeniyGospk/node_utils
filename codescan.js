...
const argv = yargs(hideBin(process.argv))
  ...
  .option("only-files", {
    alias: "of",
    type: "array",
    description: "Обрабатывать только файлы с этими именами (можно указать несколько раз)",
    default: [],
  })
  .option("include-exts", {
    alias: "ie",
    type: "array",
    description: "Расширения файлов для включения (игнорируется, если указан --only-files)",
    default: [".js", ".mjs", ".cjs", ".json", ".html", ".css", ".cs"],
  })
  .option("exclude-files", { // Проверяем эту опцию
    alias: "ef",
    type: "array",
    description: "Паттерны файлов для исключения (можно использовать glob)",
    default: [], // Убираем дефолт, чтобы точно видеть, передалось ли что-то
  })
  .option("exclude-dirs", {
    alias: "ed",
    type: "array",
...
function createIgnoreMatcher(baseDir) { // Функция теперь синхронная
    let ig;
    try {
    ...
    const selfScriptRelativePath = path.relative(baseDir, __filename);
     if (selfScriptRelativePath && !selfScriptRelativePath.startsWith('..')) {
         ...
         console.log(`[DEBUG] Добавлено правило игнорирования (скрипт): ${selfScriptRelativePath}`);
     } else {
         ...
         console.log(`[DEBUG] Добавлено правило игнорирования (скрипт, fallback): ${selfScriptPattern}`);
     }
    ...
    if (outputRelativePath && !outputRelativePath.startsWith('..')) {
        ...
        console.log(`[DEBUG] Добавлено правило игнорирования (вых. файл): ${outputRelativePath}`);
    } else {
         const outputFilenamePattern = `**/${OUTPUT_FILENAME}`;
         ig.add(outputFilenamePattern);
         console.log(`[DEBUG] Добавлено правило игнорирования (вых. файл, fallback): ${outputFilenamePattern}`);
    }
    ...
    // Убедимся, что argv.excludeDirs существует и является массивом
     if (Array.isArray(argv.excludeDirs) && argv.excludeDirs.length > 0) {
        ...
        argv.excludeDirs.forEach(pattern => {
          ...
          console.log("[DEBUG] Правила из --exclude-dirs не найдены или пусты (используются только дефолтные, если они были).");
          ...
          // Добавим стандартные сюда на всякий случай, если они могли пропасть из-за yargs
           console.log("[DEBUG] Добавляем стандартные исключения dirs на всякий случай:");
           ['node_modules', '.git'].forEach(pattern => {
               ig.add(`**/${pattern}/`); console.log(`  -> **/${pattern}/`);
               ig.add(pattern); console.log(`  -> ${pattern} (дополнительно)`);
           });
     }
    ...
    console.log(`[DEBUG] Наличие метода ig.ignores: ${typeof ig?.ignores === 'function'}`);
    console.log("-------------------------------------------");
    // --- КОНЕЦ ОТЛАДОЧНОГО ЛОГА ---
...
async function getAllCodeInDirectory(
  ...
  console.log(`--- [DEBUG] Проверка ignoreMatcher при входе в getAllCodeInDirectory (${relativeDirPath}) ---`);
  ...
  const hasIgnoresMethod = ignoreMatcher && typeof ignoreMatcher.ignores === 'function';
  console.log(`[DEBUG] Наличие метода ignoreMatcher.ignores: ${hasIgnoresMethod}`);
  console.log("-------------------------------------------------------------");
  // --- КОНЕЦ ОТЛАДОЧНОГО ЛОГА ---

  // Если ignoreMatcher не создан или сломан, прерываем сканирование этой ветки
  if (!hasIgnoresMethod) {
      console.error(`❌ [FATAL] Невалидный ignoreMatcher в директории ${relativeDirPath}. Прерываю сканирование этой ветки.`);
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      const pathToCheck = entry.isDirectory() ? `${relativePathForIgnore}/` : relativePathForIgnore;
      ...
      if (ignoreMatcher.ignores(pathToCheck)) {
          ...
          console.log(`   -> Игнорирую ${type} (правило): ${relativePathForIgnore}`);
...
async function run() {
  ...
  if (IS_ONLY_FILES_MODE) {
    ...
    console.log(`Включаемые расширения: ${INCLUDE_EXTENSIONS.join(", ")}`);
  }
  // Логи для переданных исключений
  console.log(`Исключаемые файлы (передано): ${argv.ef?.join(', ') || 'Нет'}`); // Используем короткое имя 'ef'
  console.log(`Исключаемые директории (передано): ${argv.ed?.join(', ') || 'Нет'}`); // Используем короткое имя 'ed'
  console.log("---------------------------\n");
  ...
  try {
    ...
    // Дополнительная проверка после создания
     if (!ignoreMatcher || typeof ignoreMatcher.ignores !== 'function') {
         console.error("❌ [FATAL] Не удалось создать корректный ignoreMatcher. Выполнение прервано.");
    ...
    if (count === 0 && content.length === 0) { // Проверяем и контент на случай ошибок
